...
export function PanelVisibilityProvider({ children }: { children: React.ReactNode }) {
  const [visibility, setVisibility] = useState<Record<string, boolean>>({})

  const show   = useCallback((id: string) => setVisibility(v => ({ ...v, [id]: true })), [])
  const hide   = useCallback((id: string) => setVisibility(v => ({ ...v, [id]: false })), [])
  const toggle = useCallback((id: string) => setVisibility(v => ({ ...v, [id]: !(v[id] ?? true) })), [])
