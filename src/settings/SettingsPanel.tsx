...
const COLOR_TOKENS = [
  ...
  '--color-fg-primary', '--color-fg-muted', '--color-fg-disabled',
...
function SettingsSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-6 last:mb-0">
      <p className="text-xs font-semibold uppercase tracking-widest text-[--color-fg-disabled] mb-3">
        {title}
      </p>
      {children}
    </div>
...
export function SettingsPanel({ expose, renderAppSettings, className }: SettingsPanelProps) {
  const { mode, setMode, colors, setColors, shape, setShape, motion, setMotion, depth, setDepth } =
    useC7One()
  ...
  return (
    <Card
      ...
      variant="elevated"
    >
      <p className="text-sm font-semibold text-[--color-fg-primary] mb-5">Settings</p>
            ...
            {BUILT_IN_THEMES.map(t => (
              <button
                ...
                className={cn(
                  ...
                  'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[--color-accent] rounded-sm',
                )}
              >
                {/* mini swatch */}
                <span
                ...
                />
                <span className="text-[10px] text-[--color-fg-muted] capitalize">{t.id}</span>
              </button>
            ))}
          </div>
          ...
          <div className="mt-3 space-y-2">
            {COLOR_TOKENS.map(token => (
              <div key={token} className="flex items-center justify-between gap-2">
                <Label className="text-[10px] text-[--color-fg-muted] truncate flex-1" title={token}>
                  {token.replace('--color-', '')}
                </Label>
                <input
                  type="color"
                  value={colors[token as keyof ThemeTokens] ?? '#000000'}
                  onChange={e => setColors({ [token]: e.target.value } as Partial<ThemeTokens>)}
      ...
      {/* ── Shape ─────────────────────────────────────────────────────── */}
      {(isExposed(expose, 'shape.radius') || isExposed(expose, 'shape.borderWidth')) && (
        <SettingsSection title="Shape">
          {isExposed(expose, 'shape.radius') && (
            <div className="mb-3">
              <Label className="text-xs text-[--color-fg-muted] mb-1.5 block">
              ...
              <Slider
                ...
                onValueChange={([v]) => setShape({ radius: `${(v / 100).toFixed(2)}rem` })}
              />
            </div>
          )}
